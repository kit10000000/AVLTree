...
int main(int argc, const char * argv[])
{
    ...
    if (argc >= 1)// если параметры введены, то инициализируем узлы, иначе выводим просто меню
    {
        AVLTree::Tree tree;
        bool check = check_repeating(argc, argv); //проверка на уникальность ключей
        std::cout << check << std::endl;
        if(check)
        {
            std::cout << "Введённые данные содержат повторяющиеся элементы" << std::endl;
            return 0;
        }
        else
        {
...
bool check_input(int input)
{
    ...
    is_correct =  input > 8 or input < 1 ? 0 : 1;
    return is_correct;
}
    ...
    while(c!= 8)
    {
        ...
        switch(c)
        {
            ...
            case 8:
                ...
                break;
            default:
                std::cout << "Не является пунктом меню, попробуйте снова" << std::endl;
        }
    }
}
